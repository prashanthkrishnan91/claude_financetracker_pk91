...
- Use after failed tests, failed builds, failed log fetches, failed PR checks, failed provider / runtime calls, failed SQL apply.
...
- For decision/snapshot/action paths, escalate to `policy-authority-reviewer` if visible decision authority might be affected.
