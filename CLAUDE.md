...
For every non-trivial implementation, bug fix, UI change, provider/runtime change, SQL/migration change, PR review, or workflow update, use the repo AI Operating System before coding:

1. Read `docs/ai/AI_REPO_OPERATING_SYSTEM.md`.
2. Read `docs/ai/KNOWN_FAILURE_MODES.md`.
3. Use focused skills in `.claude/skills/*/SKILL.md` for planning, contract audit, test selection, runtime/SQL gates, claim/data safety, self-audit, PR summary, and failure recovery.
4. Use read-only reviewer agents in `.claude/agents/*.md` for independent contract, test, policy-authority, Data Truth, SQL/runtime, plain-English UI, and PR review when applicable.
...
- `.claude/skills/runtime-gate/SKILL.md` — provider/runtime/SQL/snapshot/env evidence
- `.claude/skills/claim-safety-gate/SKILL.md` — visible text/data/evidence/decision safety
...
- `.claude/skills/failure-recovery/SKILL.md` — failed patch/review/runtime/SQL recovery
...
- UI must stay plain-English: no raw metric keys, diagnostics, shadow labels, posture labels, or advanced jargon leakage.
