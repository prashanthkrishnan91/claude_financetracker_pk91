...
Do **not** split simply because a task includes related tests, docs, contract, and code; if those belong to one coherent capability, ship one slice. Avoid six micro-phases when one guarded backend slice is safe.
...
- State **test tier used** and **why it was sufficient** (see `TEST_ROUTING.md`).
