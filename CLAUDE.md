...
Do **not** split simply because a task includes related tests, docs, contract, and code; if those belong to one coherent capability, ship one slice.
...
OS v4 is the active operating system. It absorbs OS v2 and OS v3, and consolidates prompt compression, safety packs, capability slices, batch-vs-split, and tiered tests. Do not introduce v4.2 or v5 labels — extend OS v4 in place.

For every non-trivial implementation, bug fix, UI change, provider/runtime change, SQL/migration change, PR review, or workflow update:
...
9. Stop and propose a split only when the durable fix genuinely exceeds the current capability slice.
...
- After one failed patch, reclassify. After two related patches, stop patching — escalate to full-plumbing analysis or split.
- Subagents: bounded read-only investigation/review to protect main context. No agent spam. No parallel fan-out for Level 1 work.
- Level 2/3 risky PRs: require a targeted fresh-context reviewer note or explicit "no reviewer — <reason>".
- Design overhaul PRs: classify foundation-only / visible adoption / polish. Foundation-only must lead to visible adoption. Do not call invisible infrastructure "visual transformation."
- Keep CLAUDE.md under 200 lines. Enforcement belongs in scripts/templates/hooks/CI, not in prompts.
...
- `docs/ai/TEST_ROUTING.md` — test tier hierarchy
...
Useful command aliases (call only when relevant): `/test-selector`, `/contract-audit`, `/latency-gate`, `/claim-safety-gate`, `/pre-pr-self-audit`, `/pr-summary`, `/update-handoff`, `/workflow-retrospective`, `/miss-ledger-update`, `/prompt-intake`, `/roadmap-check`, `/idea-triage`, `/progress-report`, `/build-queue-update`, `/ai-pr-readiness`.

## Core rules

- Default to one coherent capability slice; split only on the criteria above.
- Default test routing follows `docs/ai/TEST_ROUTING.md`. Every PR summary states **test tier used** and **why this tier was sufficient**.
...
Finance-specific safety lives in named packs in `docs/ai/SAFETY_PACKS_AND_ARCHETYPES.md` (Deterministic Decision Authority Pack, Valuation Safety Pack, Data Truth / Evidence Suppression Pack, Deploy/Watchtower Boundary Pack). Prompts should reference the pack name; they should not re-list the rules.
