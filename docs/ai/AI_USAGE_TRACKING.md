...
- **Level 1+ PRs always require a committed ledger row in `docs/ai/USAGE_LEDGER.md`.** "Usage unavailable" does not waive this requirement.
- If ccusage/tooling is unavailable, a manual ledger row is still required in `docs/ai/USAGE_LEDGER.md` with all metadata fields filled (Prompt ID, Phase, Model, Chat strategy, Main drivers, Waste classification, Follow-up count). Mark only the token count and delta columns as `unavailable`.
- Only Level 0 docs-only PRs may skip the ledger row entirely.
