# Context Management — Finance Tracker
...
- The task touches multiple contracts: policy, snapshot, Data Truth, worker, SQL, or UI.
...
- unresolved blockers
- manual actions (SQL/env/runtime/Vercel/Railway)
- next required action
...
- Finance-specific reviewers only when their domain is touched:
...
- Certification pack only when applicable:
  ...
  - `premium-delight-reviewer` for design-polish surfaces
