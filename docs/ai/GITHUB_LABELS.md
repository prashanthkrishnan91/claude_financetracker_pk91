...
- Use `ai:runtime-validation-needed` only when local tests or SQL cannot prove production behavior.
- Use `ai:no-ui-validation-yet` to prevent unnecessary manual UI checks after backend-only/internal/shadow/docs PRs.
