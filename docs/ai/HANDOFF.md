...
- Roadmap stage: **Stage 2.5B — Deploy v3 snapshot market-value source v1 complete**. `PortfolioService.create_snapshot()` now enriches each `positions_data` entry with `market_price_usd`, `market_value_usd`, `market_value_source`, and `market_value_certified_at` when a valid non-stale current price is available from the price engine. Cost basis is never stored under `market_value_usd`. Missing/stale/invalid prices → field omitted → Deploy adapter sees MISSING trust (fail-safe preserved). Reuses the price service's 5-minute in-memory cache; no second provider fetch. Price fetch failure falls back gracefully — snapshot stored without market values. 25 new backend tests (12 unit, 6 integration, 7 adapter integration); 0 failed.
- Active build queue item: **Deploy v3 exact-dollar path completion** — snapshot market values are now written (Stage 2.5B complete). `exact_dollar_ready` can be True in production when: (1) fresh snapshot with valid prices (now done), (2) complete target allocations exist, (3) `deploy_minimum_trade_usd` + `deploy_rounding_policy` set. Remaining gates: complete target allocations and policy config. Deploy Stage 2 exit gate is within reach once those are set.
...
**Deploy v3 exact-dollar path completion** (still inside Stage 2). Stage 2.5B is complete — snapshots now store `market_value_usd` per position. The two remaining gates for `exact_dollar_ready=True` in production are: (1) **complete target allocations** — user must have `target_allocations` rows for every portfolio position ticker, and (2) **policy env vars** — `deploy_minimum_trade_usd` + `deploy_rounding_policy` must be set. Next decision: whether to build a UI or admin flow for setting target allocations, or validate against production data first. Until both gates are met, BUY/TRIM items remain `not_ready`. Watchtower trigger foundation stays in Next/Later until Deploy has a certified action-plan path.
