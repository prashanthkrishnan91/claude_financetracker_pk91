...
Last updated: 2026-05-25 (Stage 9F.2a — HTTP diagnostic endpoint for no-shell validation, PR #418). **What changed (this PR):** Added `POST /api/v1/diagnostics/finance-intel/etf-nport-live-check` diagnostic endpoint so the owner can validate NPORT CIK resolution and holdings parsing without a Railway/CLI shell. New modules: `nport_diagnostic_runner.py` (pure, injectable runner — no FastAPI/Supabase dependency) + 13 new tests (64-76) for the runner. Config flag: `intel_v3_nport_diagnostic_endpoint_enabled: bool = False` (default OFF). Auth: existing `_get_runtime_cert_user` guard (requires `FINANCE_RUNTIME_CERT_ENABLED=true` + `X-Finance-Runtime-Cert-Secret` header). **Usage after deploy:** POST `https://<railway-url>/api/v1/diagnostics/finance-intel/etf-nport-live-check` with header `X-Finance-Runtime-Cert-Secret: <secret>` and body `{"tickers":["SPY","QQQ","VOO","VTI","VGT","VHT","VIS","VXUS","VYM","SCHD"]}`. Compact per-ticker JSON returned — no raw XML, no artifact writes, no decision changes. **Governance invariants:** `safe_for_decision=false`, `artifact_writes=0`, `decision_policy_changed=false`, `synthesis_ready_changed=false`. Log key: `nport_live_diagnostic_complete total=N success=N no_data=N error=N`. **To enable:** set `INTEL_V3_NPORT_DIAGNOSTIC_ENDPOINT_ENABLED=true` in Railway. **Previous patch (PR #417):** SGML extraction fix for SPY/QQQ/XLE `no_holdings_found`; `filing_not_parseable` discrimination; `NPORT-P/A` support; diagnostic payload fields in thin artifacts; removed wrong VHT/VIS/VXUS CIKs; 8 tests (56-63); `scripts/validation/nport_live_check.py`. **Remaining blockers:** Vanguard ETFs (VOO/VGT/VYM, VHT/VIS/VTI/VXUS) and SCHD need parent-registrant CIK verification (run the new endpoint after deploy with flag on + SEC_EDGAR_USER_AGENT). Production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF until at least one Vanguard ETF returns success. **Previous:** Stage 9F.2a v1 — basic provider/adapter/runner added (PR #416 merged). Next: Enable diagnostic endpoint, verify CIKs via HTTP, then enable production flag.
