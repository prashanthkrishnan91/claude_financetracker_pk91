...
Last updated: 2026-05-25 (Stage 9F — ETF Fund Intelligence Dataset v1, PR #414 merged). **What changed:** New pure module `canonical_etf_fund_dataset_v1.py` builds the first honest ETF canonical scaffold. `build_canonical_etf_fund_dataset_row()` is pure no-IO. Non-ETF → `etf_applicable=False`, `canonical_etf_dataset_safe=False`. ETF with usable fundamentals → fund identity/cost/yield PARTIAL; ETF with no fundamentals → MISSING. Composition always MISSING (no dedicated fund data provider at Stage 9F). `etf_fund_intelligence_ready=False` always. `canonical_etf_dataset_safe=True` for all ETF rows (scaffold built). **Forensics integration:** New `BUCKET_ETF_FUND_COMPOSITION_NOT_READY` bucket (12 total); ETFs with scaffold built land here instead of `ETF_PROVIDER_NOT_BUILT`. `HoldingForensicsRow` gains `canonical_etf_dataset` field (ETF only). `DataFoundationForensicsResult` gains 4 new Stage 9F aggregates: `etf_canonical_dataset_count`, `etf_fund_intelligence_ready_count`, `etf_canonical_dataset_degraded_tickers`, `etf_missing_reason_counts`. **Asset parity roadmap:** `build_asset_parity_roadmap()` now accepts `etf_canonical_count`/`etf_fund_intelligence_ready_count`; `canonical_dataset_built=True` for ETF when Stage 9F scaffolds built. 62 new Stage 9F tests; 369 total Stage 9B/9D/9E/9E.1/9F tests pass. Stage 9C: 8 pre-existing failures (pre-Stage 9F, verified by git stash). No SQL, no LLM, no providers, no UI, no decision_policy_v1 changes. Next blocker: ETF fund data provider lane (composition/holdings/geography) OR crypto canonical dataset for parity.
