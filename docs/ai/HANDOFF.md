...
Last updated: 2026-05-24 (Stage 9E.1 — Equity Numeric Valuation Inputs v1, PR open). **What changed:** New pure module `equity_numeric_valuation_inputs_v1.py` builds per-ticker numeric valuation input readiness from `CanonicalEquityDatasetRow` + `TickerPriceSignal`. `TickerPriceSignal` per-ticker price confirmation derived from `portfolio_snapshots.positions_data` JSONB (`market_value_certified_at` → CERTIFIED, absent → CARRIED). Freshness classification from `snapshot_at` age: <24h=FRESH, 24-72h=AGING, >72h=STALE. `numeric_inputs_ready=True` gate: requires canonical_safe + ticker_level_confirmed + FRESH/AGING price + earnings AVAILABLE/PARTIAL. `valuation_interpretation_band=UNKNOWN` always (no thresholds). `safe_for_decision=False`, `synthesis_ready=False` immutable. ETF/crypto: `_build_not_applicable_inputs()` short-circuit. `equity_valuation_evidence_v1.py` updated: optional `numeric_inputs` param; `valuation_ready` now derives from `numeric_inputs.numeric_inputs_ready` when provided (was always False); Stage 9E behavior preserved when `numeric_inputs=None`. `intel_data_foundation_forensics_v1.py` extended: `_SupplementalData.ticker_price_signals` dict (default empty), `_extract_ticker_price_signals()` from snapshot row, new `DataFoundationForensicsResult` Stage 9E.1 aggregate fields. `valuation_numeric_ready=True` propagates correctly through `_classify_all_gaps` removing `VALUATION_LANE_NOT_BUILT` bucket. 297 tests pass (61 new Stage 9E.1 + 60 Stage 9E + 75 Stage 9D + 100 Stage 9B — regression-free). No SQL, no LLM, no providers, no UI, no decision_policy_v1 changes. Next blocker: ETF canonical dataset + valuation lane (parity gate). Prior: Stage 9E — Equity Valuation Evidence Lane v1 (60 tests). Prior: Stage 9D — section-level normalized evidence dataset (75 tests).
...
- **Equity:** canonical dataset built (Stage 9D ✓) + valuation evidence lane built (Stage 9E ✓) + numeric valuation inputs built (Stage 9E.1 ✓) → synthesis blocked until ETF+crypto parity
