...
**Production validation of Build 1 end-to-end batching + prewarm guard.** Re-run the Run Intel v3 button. Key log signals:

- `full_portfolio_analyst_refresh.post_run_readback selected_ticker_count=N` must match `analyst_evidence_writer_persisted_count ... insights_written=N` (N ≤ 10 per pass, NOT 34).
- `analyst_stage.summary eligible_tickers=N` must show ≤ 10 per worker pass (scoped batch count, NOT 34).
- `intel_v3.analyst_refresh_worker_run_summary` must show `claimed>0`, `succeeded>0`; succeeded accumulates across passes to reach 34.
- During intermediate passes (jobs remain): `intel_v3.analyst_refresh_worker_prewarm_deferred reason=jobs_remain` — confirms the prewarm guard is active.
- After all 34 tickers succeed across 4 bounded iterations (run_resumable=False): `prewarm_intel_v3_snapshot` triggers certification. Watch for `intel_v3_worker_certified_snapshot_published` to confirm green state — NOT before the final pass.
