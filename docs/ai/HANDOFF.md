...
- **New stock-sleeve policy guardrail (`_compute_gaps`):** Individual-stock candidates are now blocked when the whole `individual_stock` group is already at or above its 40% policy target — checked before the per-ticker evidence gate, so evidence-eligible tickers in an overweight sleeve are never misreported as evidence-missing. New `ineligibility_reason="individual_stock_group_above_target"`. Matches the live portfolio's ~52.31% individual-stock weight vs. its 40% target.
- **New diagnostic transparency fields (`stock_candidates`):** `blocked_by_policy_tickers` and `policy_block_reason_codes` (`{ticker: "individual_stock_group_above_target"}`) added alongside the existing `blocked_by_evidence_tickers`. `stock_candidates.status` now correctly reports `blocked_by_policy_caps` (not `blocked_insufficient_evidence`) when evidence passed but the sleeve guardrail blocked the ticker — reuses the same status value the Stage 12D/13A preview mapper (`paycheck_plan_preview.py`) already handles, so no preview-router changes were needed.
...
- **Files changed:** `v2/backend/app/services/allocation_policy_v1.py` (`_extract_intel_v3_cards`, `_intel_v3_snapshot_fallback_timestamp`, sleeve guardrail, diagnostic fields), `v2/backend/tests/test_allocation_policy_v1.py` (13 new tests), `docs/ai/HANDOFF.md`, `docs/ai/USAGE_LEDGER.md`.
