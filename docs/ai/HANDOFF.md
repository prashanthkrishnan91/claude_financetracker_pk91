...
Last updated: 2026-05-25 (Stage 9F.2a patch — SEC NPORT-P provider fix: SGML extraction + diagnostics). **What changed (patch):** Fixed two production root causes: (1) EDGAR `primaryDocument` for large ETFs (SPY/QQQ/XLE) is often a complete SGML submission text file, not standalone XML — `ET.fromstring` fails silently, producing `no_holdings_found`. Fixed by `_extract_xml_from_sgml_submission()` helper that extracts the embedded XML from within `<TEXT>...</TEXT>` of the NPORT document section. `_parse_nport_xml` now returns 3-tuple `(holdings, fund_meta, parse_status)` where `parse_status` distinguishes `xml_parse_error` / `no_holdings_container` / `ok`. (2) `no_holdings_found` was previously returned for both XML parse errors AND genuinely empty holdings — now `filing_not_parseable` is returned for parse failures (`xml_parse_error`, `no_holdings_container`). Added `NPORT-P/A` / `NPORT-EX/A` to accepted form types. Added `primary_doc_attempted`, `parse_failure_stage` fields to `NportProviderResult`. Thin no-data artifact now includes `resolved_cik`, `primary_doc_attempted`, `parse_failure_stage`, `accession_number` for actionable diagnostics. CIK seed map: removed confirmed-wrong VHT/VIS/VXUS entries (fall through to company_tickers.json); documented Vanguard parent-registrant CIK requirement. Added 8 new tests (56-63) including SPY SGML fixture → 3 holdings (proves fix), full pipeline → FactRecords > 0, Vanguard-style XML, NPORT-P/A, diagnostic payload fields. Added `scripts/validation/nport_live_check.py` (gated manual script). 130 tests pass total. **Remaining blockers:** Vanguard ETFs (VOO/VGT/VYM, VHT/VIS/VTI/VXUS) need parent-registrant CIK verification via `nport_live_check.py` — SCHD also needs verify. Flag remains OFF. **Previous:** Stage 9F.2a v1 — basic provider/adapter/runner added (PR #416 merged). Next: Enable flag in Railway (with verified CIKs) OR ETF sector/geography derivation (9F.3) OR crypto canonical dataset.
