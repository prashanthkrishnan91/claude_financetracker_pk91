...
Last updated: 2026-05-26 (Stage 9F.2a — ETF NPORT multi-filing scan, open PR). **What changed:** Multi-filing scan added to NPORT provider to fix XLE/VGT/VHT/VIS returning `series_identity_not_proven` when the *latest* parent-registrant NPORT-P filing belongs to a different series (e.g., SPDR China ETF for SPDR Series Trust, or Vanguard Value Index Fund for Vanguard World Fund). Root cause: multi-series registrants file one NPORT-P per series; the latest filing may belong to any series in the registrant. **Fix:** (1) `nport_provider_v1.py`: replaced `_find_latest_nport_filing()` with `_collect_recent_nport_filings(submissions_body, max_filings)` returning up to N filings newest-first; inner filing-scan loop: identity mismatch → `continue` to next filing (not next candidate); budget check fires before each document fetch; `_scan_limit_reached=True` + `break` when budget exhausted; after all candidates exhausted returns `series_identity_scan_budget_exhausted`. Constants: `_DEFAULT_FILING_SCAN_CAP=12`, `_MAX_REQUESTS_PER_TICKER` raised 5→20. New `NportProviderConfig.max_filings_to_scan`. New `NportProviderResult` fields: `filings_scanned_count`, `matching_filing_rank`, `scan_limit_reached`. (2) `nport_diagnostic_runner.py`: 3 scan fields in `_build_ticker_entry`; config uses explicit `max_filings_to_scan=12`. (3) `test_stage9f2a_nport_filing_scan.py`: 31 new fixture-based tests covering match on filing #2, budget exhaustion, XLE/VGT/VHT/VIS scan past wrong series, standalone trust unchanged (matching_filing_rank=1), GLD unchanged, scan field propagation, scan cap. **65 total Stage 9F.2a tests pass (34 identity-cert + 31 filing-scan). Status: diagnostic validation only** — production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF. Identity cert invariant preserved: no holdings returned unless identity_verified=True or standalone_trust=True. **Post-deploy validation:** POST diagnostic endpoint — XLE should return `success_identity_verified` with `matching_filing_rank≥1`; VGT/VHT/VIS find their series within scan window; GLD shows `commodity_trust_or_no_nport_data`.
