...
Last updated: 2026-05-26 (Stage 9F.2a — ETF NPORT resolver discovery, open PR). **What changed (current PR):** Stops one-CIK-per-ticker static guessing by adding SEC-backed candidate discovery in the diagnostic lane. Runtime diagnostic after PR #423 showed: XLE scanned 12 filings under CIK 0001168164 but found unrelated SPDR series; VGT/VHT/VIS scanned 12 filings under 0000036405 but detected VANGUARD INDEX FUNDS (not VANGUARD WORLD FUND); VOO/VTI/VXUS no_nport_filing; VYM sec_error/404. Root cause: candidate CIK quality for several ETFs. **Fix (diagnostic-only, no production changes):** (1) New `etf_nport_candidate_discovery.py`: `discover_nport_candidates(ticker, entry, *, http_get_fn=None)` queries SEC EDGAR EFTS (`https://efts.sec.gov/LATEST/search-index`) by registrant name (entity search) and expected series name (series search). Returns `NportDiscoveryResult` with `NportCandidateEntry` list (confidence: confirmed_candidate / plausible_candidate / rejected). Static CIKs excluded from discovered_candidates (in existing_static_candidates). All HTTP calls injectable for tests. (2) Enhanced `nport_diagnostic_runner.py`: `run_nport_live_check()` gains optional `discovery_fn` and `discovered_provider_fn` params. When discovery_fn provided and identity not verified: calls discovery → filters non-rejected candidates not yet tried → calls discovered_provider_fn with new CIKs. Per-ticker output gains: `resolver_discovery_used`, `resolver_discovery_candidates`, `resolver_discovery_sources_tried`, `resolver_discovery_error`. Top-level gains: `resolver_discovery_enabled`. (3) 23 new fixture-based tests in `test_stage9f2a_nport_resolver_discovery.py` (tests 84–106). Zero live SEC calls in CI. 217 total Stage 9F.2a tests pass. **Invariants preserved:** production NPORT evidence flag remains OFF; no holdings without identity_verified=True; SPY/QQQ standalone trust unchanged; GLD commodity trust unchanged; no SQL/UI/LLM/paid provider/decision/synthesis/Deploy changes. **Post-deploy runtime validation required** — see PR body for instructions. Static map not updated in this PR; update requires post-deploy identity confirmation of discovered candidates.
