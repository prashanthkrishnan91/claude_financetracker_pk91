...
Last updated: 2026-06-11 (Stage 10A — product spine reality audit; docs-only; next-PR sequence chosen from code evidence).

**Stage 10A product spine reality audit (current PR):** Docs/audit only — no code, UI, SQL, provider migration, synthesis, or decision-policy changes. New `docs/ai/PRODUCT_SPINE_REALITY_AUDIT.md` maps the revised spine onto the actual v2 code. Evidence standard: static code/docs inspection — identifies implemented paths and gaps; does not certify runtime behavior unless runtime evidence is explicitly cited.
- **Implemented / code-wired today (not runtime-certified by this PR):** F1 Books of Record (positions/transactions/deposits/DRIP/Plaid/CSV — AVCO cost basis, Plaid authoritative w/ 24h cache, CSV dedup+reconcile), F2 Data Backbone (concurrent price engine yfinance+Finnhub+Alpaca+Polygon+CoinGecko, circuit breakers), Stage 1 Honest Intel (deterministic `decision_policy_v1`, suppression/freshness/credibility/governance — most mature), Stage 4 labeled advisory analyst (single-LLM orchestrator, conviction capped, action deterministic), Stage 2 Deploy exact-dollar math (documented Stage 2 exit per repo docs, $900/$1,500; not revalidated here).
- **True gaps:** (1) Tax/wash-sale safety absent — `tax_guardrail_status`/`wash_sale_guardrail_status` = `"not_evaluated_yet"`; items terminate in `ACTIONABLE_PENDING_TAX`; no tax-lot model. (2) VTI-only benchmark of truth absent — `benchmark.py` is SPY *signal input only*, not a user-facing DCA comparison. Watchtower real-send parked (Resend domain). Teaching/polish not built (correctly gated).
- **yfinance:** NOT a sole critical-path dependency (one of several concurrent price sources; evidence-only elsewhere). **Do not migrate to Finnhub.**
- **Books:** sufficient to operate; needs a read-only reconciliation *audit* surface (not a repair) before building Benchmark/Tax on top.
- **Next-PR sequence (from code evidence):** PR-1 Stage 10B books integrity/reconciliation diagnostic (read-only) → PR-2 Stage 10C VTI-only DCA benchmark (read-only, deterministic) → PR-3 Stage 10D Deploy tax-lot/wash-sale source design+contract (design PR, no behavior change) → PR-4 (optional) Stage 10E readiness-flag read model. No Finnhub migration, no UI polish, no more ETF proof.
- **Recommended next PR:** Stage 10B — Books-of-record integrity & reconciliation diagnostic (read-only), to establish `facts_ready` before downstream build.
- **Proposed readiness flags (not implemented):** facts_ready, advisory_ready, deployment_safe (False today — tax gap), decision_ready, canonical_ready. `safe_for_decision` semantics unchanged. ETF holdings remain enrichment/exposure evidence only.
