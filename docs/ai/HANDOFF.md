...
Last updated: 2026-05-22 (Stage 8D — SEC/company catalyst sentiment evidence readiness surface in Intel drawer. New `catalyst_display_adapter_v1.py` translates Stage 5K AxisReadinessSignal into three safe boolean display fields (sec_catalyst_found, editorial_suppressed, sec_lane_applicable). `intel_v3_service.py` injects `sec_catalyst_display` into `research_axis_readiness` in both run paths. `snapshot_builder.py` embeds `sec_catalyst_evidence` in `evidence_explanation`. Frontend: new `SecCatalystEvidenceDisplay` type, `buildCatalystEvidenceDisplay()` helper, and `CatalystEvidenceModule` component in `IntelV3Drawer`. ETFs/crypto: sec_lane_applicable=False → no display. No raw backend codes in UI. No policy change. 18 backend + 21 frontend tests. No SQL.)
...
- 2026-05-22 — **Stage 8D: SEC/company catalyst evidence readiness UI surface** — New `catalyst_display_adapter_v1.py` converts Stage 5K `AxisReadinessSignal` into three safe boolean display fields (sec_catalyst_found, editorial_suppressed, sec_lane_applicable). `intel_v3_service.py` injects these into `_research_axis_readiness` in both `enqueue_run_v3` and `run_prewarm_snapshot` paths. `snapshot_builder._build_held_card()` embeds `sec_catalyst_evidence` in `evidence_explanation`. Frontend: `SecCatalystEvidenceDisplay` type in `api.ts`; `buildCatalystEvidenceDisplay()` in `intel-v3-explanation.ts`; `CatalystEvidenceModule` component inside Evidence Check section of `IntelV3Drawer`. ETFs/crypto/non-equity: sec_lane_applicable=False → module hidden. No raw backend codes visible. No decision policy change. 18 backend + 21 frontend tests. No SQL, no providers, no LLM.
