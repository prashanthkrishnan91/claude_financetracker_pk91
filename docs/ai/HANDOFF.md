...
Last updated: 2026-05-25 (Stage 9F.2a — filing index fallback chain, open PR). **What changed:** Root-cause fix for SPY/QQQ/XLE `parse_failure_stage=index_fetch_error` after PR #419. PR #419 detected XSL viewer `primaryDocument` paths correctly but tried `{acc_raw}-index.json` which returns 404. Fix: replaced single URL assumption with robust 3-step fallback chain — A. `{folder}/index.json` (directory.item format), B. `{folder}/{acc_raw}-index.html` (HTML index), C. `{folder}/{acc_nodash}.txt` (complete SGML, direct parse). `_select_best_nport_doc_from_index()` now handles both `documents` (old filing-specific JSON) and `directory.item` (new folder listing JSON) formats. New helpers: `_parse_index_html_links()`, `_select_best_nport_doc_from_html_links()`. New diagnostic field: `index_urls_attempted_count`. Budget raised to 5. 8 new tests (84-91) + all 72 prior tests pass. **3 files changed**: `nport_provider_v1.py`, `nport_diagnostic_runner.py`, `test_stage9f2a_etf_nport_holdings.py`. **Post-deploy validation:** POST diagnostic endpoint with `{"tickers":["SPY","QQQ","XLE","GLD"]}` — expected: SPY/QQQ/XLE `fetch_status=success`, `selected_doc_source=index_json`, `holdings_count>0`; GLD `commodity_trust_or_no_nport_data`. **Does NOT fix:** Vanguard/SCHD parent CIK resolution (separate slice). Production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF.
