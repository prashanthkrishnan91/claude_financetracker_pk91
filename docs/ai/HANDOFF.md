...
- 2026-05-15 — **Build 1.5: Intel v3 sub-10-second user-facing experience** — Root cause of multi-minute UX: worker loop ran ONE batch per 60-second sleep. With 34 tickers / 10 per batch = 4 batches × 60s gap = minutes. Fix: (1) `_drain_cycle()` added to entrypoint — runs multiple batches in one cycle when `run_resumable=True` and budget allows (max 8 batches / 300s wall cap); (2) `intel_v3.analyst_refresh_worker_drain_cycle_summary` log emits `worker_batches_drained`, `worker_drain_total_duration_ms`, `worker_idle_delay_skipped`, `time_to_worker_certified_snapshot_ms`; (3) `get_latest_snapshot()` emits `snapshot_response_ms`; (4) `enqueue_run_v3()` emits `run_click_response_ms`, `certified_snapshot_available_on_click`, `refresh_jobs_pending_count`, `refresh_jobs_remaining_count`; (5) banner `refreshing_analyst_intelligence` copy updated (removed false "60 seconds" claim). Build 1 trust guarantees intact: prewarm deferred until all jobs drained, certification contract unchanged, no fake freshness. 13 new backend tests (154 total) + 10 new frontend banner tests (36 total). No SQL, no schema changes, no certification weakening.
