...
- **New endpoint:** `POST /api/v1/diagnostics/finance-intel/books-reconciliation-diagnostic` — runtime-cert-gated, body `{"tickers": [], "include_not_evaluable": true}`.
- **Output contract:** `diagnostic_version`, `facts_ready` (global bool), `positions_facts_ready_count`, `positions_degraded_count`, `positions_blocked_count`, per-ticker reconciliation with status (`facts_ready`|`degraded`|`blocked`|`not_evaluable`), `blockers[]`, `warnings[]`, `rationale`. `diagnostics_only=true`, `writes_performed=0`, `policy_unchanged=true`, `visible_snapshot_unchanged=true`.
- **Reconciliation rules (conservative):** No position → blocked. Zero/negative shares → blocked. No tx history + position exists → degraded (Plaid/manual may be authoritative). Crypto positions → not_evaluable (AVCO reconciliation inapplicable). Tx qty drift >10% → blocked. Tx qty drift 1–10% → degraded. Cost basis drift >2% → degraded warning. Global `facts_ready=true` requires zero blocked and zero degraded.
- **Tests:** `test_books_reconciliation_diagnostic_v1.py` — 25/25 pass. All DB calls mocked; no live I/O.
- **How to run after merge:** Set runtime-cert header, then: `POST /api/v1/diagnostics/finance-intel/books-reconciliation-diagnostic` with `{"tickers": [], "include_not_evaluable": true}`. Inspect `facts_ready` and `per_ticker[].reconciliation_status`. If any ticker is blocked, read `blockers[]`; if degraded, read `warnings[]`.
- **What unlocks Stage 10C:** If runtime result shows `facts_ready=true` (all positions facts_ready or not_evaluable/safe), proceed to Stage 10C VTI-only DCA benchmark. If result shows degraded/blocked, classify: are the gaps repairable with a books-repair PR, or is the benchmark viable on a subset of facts_ready positions only? Do not build the benchmark on blocked data.
