...
Last updated: 2026-06-24 (Stage 12C — Core ETF Preference + Paycheck Allocation Output Polish; backend/tests/docs only; no UI, no SQL, no LLM, no provider calls).

**Stage 12C — Core ETF Preference + Paycheck Allocation Output Polish (current PR):** Backend/tests/docs only. No UI, no SQL migration, no LLM calls, no provider calls, no writes. Refines the Stage 12B allocation policy so broad-market ETF purchases use a deterministic core ETF preference order rather than raw gap size.
...
- **Sort key change:** 3-tuple `(group_priority, conviction_rank, gap_pct)` → 4-tuple `(group_priority, conviction_rank, core_preference_rank, gap_pct)`. Sorted descending. Within same group/conviction tier, preference rank overrides gap size.
...
- **Tests:** 92/92 pass (78 pre-existing + 14 new `TestCoreETFPreference` + 7 new `TestStage12CRegressionFixture`). Regression fixture has AAPL+NVDA+MSFT dominant stocks (~85% of portfolio) with VTI having smaller gap than SPY — confirms VTI is selected first despite SPY's larger gap.
