...
Last updated: 2026-05-26 (Stage 9F.2a — ETF NPORT identity-certification repair, **PR #422 merged**). **What changed:** Identity certification lane added to NPORT provider to prevent false positives where VGT/VHT/VIS all returned `success` from the same parent registrant filing (CIK 0000036405) without verifying which series the filing belongs to. Also fixed candidate iteration semantic bug (Blocker 2): previously returned `series_identity_not_proven` immediately on identity mismatch; now records failure in `candidate_identity_failures` and continues to next candidate. **Fix:** (1) `etf_parent_cik_resolver.py` extended with 4 new fields: `expected_series_names`, `standalone_trust`, `commodity_trust`, `candidate_ciks`. (2) `nport_provider_v1.py`: `_normalize_name()` strips punctuation (U.S.→us=US→us); `_identity_name_matches()` bidirectional normalized substring; identity statuses: `success_identity_verified`, `success_identity_assumed_single_series`, `series_identity_not_proven` (no holdings), `commodity_trust_or_no_nport_data`. 12 new fields on `NportProviderResult` including `candidate_identity_failures`. (3) `nport_diagnostic_runner.py` surfaces all 12 identity fields + `tickers_identity_verified` aggregate. (4) `test_stage9f2a_nport_identity_cert.py` — 34 fixture-based tests (84-112) including tests 111-112 for multi-candidate identity resolution. **145 total Stage 9F.2a tests pass.** **Status: diagnostic validation only** — production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF. **Post-deploy validation:** POST diagnostic endpoint — SPY/QQQ/XLE return `identity_status=success_identity_*`; VGT/VHT/VIS show `series_identity_not_proven` or `success_identity_verified`; GLD shows `commodity_trust_or_no_nport_data`; log key: `identity_status`.
