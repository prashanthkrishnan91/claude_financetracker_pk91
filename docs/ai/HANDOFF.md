...
Last updated: 2026-05-25 (Stage 9F.2a — ETF parent-registrant CIK resolver, open PR). **What changed:** Added `etf_parent_cik_resolver.py` — a new module that maps ETF tickers to the correct parent-registrant CIK for NPORT-P filing lookup. Root cause: Vanguard/Schwab ETF tickers map to series/share-class CIKs in company_tickers.json (no NPORT-P filings); the correct filer is the umbrella parent registrant (e.g. VANGUARD INDEX FUNDS, VANGUARD WORLD FUND). VOO/VTI now use CIK 0000764180 (VANGUARD INDEX FUNDS); VGT/VHT/VIS use 0000036405 (VANGUARD WORLD FUND); VYM uses 0000916548 (VANGUARD WHITEHALL FUNDS); SCHD uses 0001477379 (SCHWAB STRATEGIC TRUST); VXUS uses candidate CIK 0001004244 (VANGUARD INTERNATIONAL EQUITY INDEX FUNDS). VHT/VIS/VXUS (previously missing_cik) now have parent map entries. CIK resolution in `fetch_etf_nport_holdings` now: 1. checks `etf_parent_cik_resolver.get_parent_registrant_entry()`, 2. falls back to company_tickers.json for unknown tickers. Old `_ETF_CIK_SEED_MAP` replaced/superseded by parent map. Two new diagnostic fields on `NportProviderResult`: `resolver_source` ("etf_parent_map" | "company_tickers" | "injected") and `parent_registrant_name`. Both surfaced in diagnostic runner entry. 33 new tests (51-83) pass; all 80 existing stage-9f2a tests preserved; all 13 diagnostic endpoint tests preserved. **4 files changed**: `etf_parent_cik_resolver.py` (new), `nport_provider_v1.py`, `nport_diagnostic_runner.py`, `test_stage9f2a_etf_parent_cik_resolver.py` (new). **Status: diagnostic validation only** — parent CIKs for Vanguard/SCHD are candidates; post-deploy validation required. Production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF. **Post-deploy validation:** POST diagnostic endpoint with `{"tickers":["VOO","VTI","VGT","VHT","VIS","VXUS","VYM","SCHD"]}` — success criterion: at least one returns `fetch_status=success, holdings_count>0`; all should show `resolver_source=etf_parent_map`; if `no_nport_filing`, check `resolved_cik` to confirm parent CIK was tried. **Preserves:** SPY/QQQ/XLE success paths, GLD commodity-trust path.
