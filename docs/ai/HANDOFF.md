...
Last updated: 2026-05-25 (Stage 9F.2a — SEC NPORT-P ETF Holdings Evidence Lane v1, PR pending). **What changed:** Added keyless SEC EDGAR NPORT-P evidence lane for ETF fund holdings. Three new files: `nport_provider_v1.py` (fetch/parse NPORT-P XML, injectable HTTP for tests, static CIK seed map for 12 ETFs, fail-closed on all error states, never raises), `etf_nport_adapter_v1.py` (pure adapter NportProviderResult→WorkerOutput; artifact_type=etf_fund_note, skill_pack=etf_sec_nport_holdings_evidence_v1; one SourceRecord per filing, one FactRecord per holding; sector_status=MISSING always; geography_status=AVAILABLE when countryOfRisk present; safe_for_decision=False; no raw XML in payload). Runner: `run_etf_nport_holdings_evidence()` added to `evidence_lane_runner_v1.py` (ETF-only guard via `_classify_holding_as_etf`; kill-switch hierarchy: global → per-lane → user-agent check; skips write when holdings_count=0). Registry: `LANE_ETF_FUND_DATA` added to `evidence_provider_registry_v1.py` + ALL_LANES. Config: `intel_v3_etf_nport_evidence_enabled: bool = False` (default OFF). 50 new fixture-based tests (zero live SEC calls). No SQL (etf_fund_note already in DB CHECK constraint from migration 017). No LLM, no UI, no decision-policy changes. ETF composition status remains MISSING (composition_status blocker in forensics unchanged). **NPORT limitations:** quarterly filing with ~60-day lag; GLD/commodity trusts have no equity holdings (commodity_trust_or_no_nport_data); sector labels not in NPORT (deferred to 9F.3). Stage 9F canonical scaffold (Stage 9F) unchanged — `etf_fund_intelligence_ready=False` still always. Previous: Stage 9F — ETF Fund Intelligence Dataset v1: `canonical_etf_fund_dataset_v1.py` (pure, no IO); composition MISSING; 62 tests; 369 total 9B/9D/9E/9E.1/9F tests pass. Next: ETF sector/geography derivation (9F.3) OR crypto canonical dataset for asset-class parity.
