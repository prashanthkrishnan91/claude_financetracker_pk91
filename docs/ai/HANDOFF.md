...
Last updated: 2026-05-12 (post Stage 2.5A Deploy v3 certified sizing source adapter v1)
...
- Roadmap stage: **Stage 2.5A — Deploy v3 certified sizing source adapter v1 wired**. `GET /api/v1/deploy/v3/plan` now calls `build_sizing_bundle_from_persisted_data` (new adapter) before planning. Adapter reads `portfolio_snapshots` + `target_allocations` + Settings policy config. Source metadata expanded: `sizing_bundle_provided`, `exact_dollar_ready`, `sizing_values_ready`, `target_allocation_ready`, `policy_ready`, `suppression_reasons`. Exact-dollar readiness requires: (1) fresh snapshot (≤24h) with `market_value_usd` per position, (2) complete target allocations for every position ticker, (3) `deploy_minimum_trade_usd` + `deploy_rounding_policy` set in env/Settings. If any gate fails, plan falls back to scaffold/not_ready — no behavior change until all three are certified.
- Active build queue item: **Deploy v3 certified sizing bundle source wiring** — adapter is wired but `exact_dollar_ready` depends on source completeness. In production, `portfolio_snapshots.positions_data` does not yet store `market_value_usd` per position (only cost basis), so `sizing_values_ready` will be False until that is addressed or a separate snapshot write path stores market values. Deploy Stage 2 exit gate remains unmet.
...
- 2026-05-12 — **Stage 2.5A: Deploy v3 certified sizing source adapter v1** — `deploy_sizing_source_adapter_v1.py` adapter reads `portfolio_snapshots` + `target_allocations` + Settings; certifies sizing inputs via deterministic staleness (24h) and explicit market-value check (cost basis never promoted). Wired into `GET /api/v1/deploy/v3/plan`; source metadata expanded with `exact_dollar_ready`, `sizing_values_ready`, `target_allocation_ready`, `policy_ready`, `suppression_reasons`. `deploy_minimum_trade_usd` / `deploy_rounding_policy` added to Settings. Fail-safe: returns None on missing snapshot or DB error; router falls back to scaffold. 51 new tests (39 adapter + 12 router); 4469 total; 0 failed. No SQL, no providers, no LLM, no UI changes.
...
**Deploy v3 exact-dollar path completion** (still inside Stage 2). The sizing adapter is wired. `exact_dollar_ready` can be True in production when: (1) a portfolio snapshot stores `market_value_usd` per position (currently not stored by `create_snapshot` — only cost basis is stored), AND (2) complete target allocations exist, AND (3) policy env vars are set. The next decision is whether to add a separate snapshot write path that includes per-position market values, or to approach this differently. Until `exact_dollar_ready` reaches True in production, BUY/TRIM items remain `not_ready`. Watchtower trigger foundation stays in Next/Later until Deploy has a certified action-plan path.
