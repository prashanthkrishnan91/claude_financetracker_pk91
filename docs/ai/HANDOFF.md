...
Last updated: 2026-05-24 (Stage 9D — Canonical Equity Research Dataset v1. **What changed:** (1) New pure module `canonical_equity_dataset_v1.py` — per-ticker read model built from existing Stage 5J LaneCoverage + supplemental fact counts. Derives availability signals for 5 operating trend inputs (revenue trend, profitability/margin, EPS/net income, FCF, share count/dilution) from SEC artifact completeness_band + observation_count proxy; never exposes raw fact values or XBRL keys. `safe_for_equity_dataset=True` only when sec_company_facts is USABLE or USABLE_WITH_LIMITATIONS for an equity ticker. `synthesis_ready`, `valuation_ready`, `safe_for_decision` are always False. ETF/crypto return NOT_APPLICABLE rows. (2) `build_asset_parity_roadmap()` — machine-readable gap summary by asset class: equity (canonical built / valuation missing / 3 SEC edge cases), ETF (fund composition/provider missing), crypto (market context/provider missing). All three asset classes must reach S-grade foundational data before synthesis can gate. (3) `intel_data_foundation_forensics_v1.py` extended: `HoldingForensicsRow.canonical_equity_dataset` field (dict or None for ETF/crypto); `DataFoundationForensicsResult.equity_canonical_dataset_count`, `.equity_canonical_dataset_degraded_tickers`, `.asset_parity_roadmap`. Valuation gap message now distinguishes: when SEC usable → "canonical equity dataset (Stage 9D) built, build valuation lane (Stage 9E) next"; when SEC not usable → "fix SEC gap first." (4) 2 Stage 9B test assertions updated to match new valuation gap message. 49 new tests in `test_stage9d_canonical_equity_dataset.py`. 280 total Stage 9A/9B/9C/9D tests pass. No SQL, no LLM, no UI, no providers, no decision_policy_v1 changes, synthesis_ready permanently False. **Production evidence (current):** 34 holdings: 19 equities, 13 ETFs, 2 crypto. 16 equities with strong SEC → safe_for_equity_dataset=True. TSM/KLAR/BLSH remain degraded. Next blocker for equities: valuation lane (Stage 9E). Prior: Stage 9C — SEC CompanyFacts readiness diagnostic + stale-artifact repair (model version bump v1→v2, 44 tests, 266 total). Prior: Stage 9B — Intel Data Foundation Forensics v1 (11-bucket root cause, 78 tests). Prior: Stage 9A — Coverage & Trust Matrix v1 (80 tests).
