...
Last updated: 2026-05-24 (Stage 9C — SEC CompanyFacts readiness diagnostic + root-cause repair. **Root cause:** Production artifacts for 18/19 equities were stale — generated before Stage 5H.3 fixed the XBRL contradiction group key, but idempotency (same model_version `sec_xbrl_companyfacts_v1`) prevented regeneration. **Fix:** (1) Bumped `_MODEL_VERSION` `sec_xbrl_companyfacts_v1` → `sec_xbrl_companyfacts_v2` in `sec_companyfacts_adapter_v1.py` — changes idempotency key so next `POST /intel/v3/run` triggers clean replacement and artifacts are re-evaluated with the correct 5H.3 contradiction group key. (2) New pure diagnostic module `sec_companyfacts_readiness_diagnostic_v1.py` — explains WHY an artifact is weak (8 categories: STRONG/LIMITED/SUPPRESSED_CONTRADICTED/SUPPRESSED_INCOMPLETE/SUPPRESSED_UNKNOWN_SOURCE/NOT_EVALUABLE/STALE_OR_UNKNOWN/MISSING); flags `is_stale_model_version` and `is_false_contradiction_candidate` (SUPPRESSED_CONTRADICTED + stale model = likely false contradiction from old key). (3) Extended `LaneCoverage` with `contradiction_count: Optional[int]` and `not_evaluable_reason: Optional[str]` extracted from stored `contradiction_assessment` payload. (4) Extended `HoldingForensicsRow` in `intel_data_foundation_forensics_v1.py` with `sec_companyfacts_diagnostic: Optional[dict]` — populated for weak equity artifacts, None otherwise. Constants: `SEC_COMPANYFACTS_CURRENT_MODEL_VERSION="sec_xbrl_companyfacts_v2"`, `SEC_COMPANYFACTS_PREV_MODEL_VERSION="sec_xbrl_companyfacts_v1"`. Strict truth gates preserved — SUPPRESSED_CONTRADICTED / NOT_EVALUABLE thresholds NOT relaxed; true same-identity contradictions still flagged. 44 new tests in `test_stage9c_sec_companyfacts_readiness.py` (XBRL identity grouping regression, diagnostic categories, LaneCoverage metadata, adapter version bump, forensics integration, safety/policy invariants). 2 tests updated in `test_stage5h_sec_companyfacts_adapter.py` to reflect v2. 266 tests pass across Stage 5H/5H2/5H3/9B/9C. No SQL, no LLM, no UI, no providers, no decision_policy_v1 changes, no raw fact values exposed, safe_for_decision=False throughout. Prior: Stage 9B — Intel Data Foundation Forensics v1. `intel_data_foundation_forensics_v1.py` (pure, read-only): per-holding 11-bucket root cause classification (ETF_PROVIDER_NOT_BUILT → … → DATA_PRESENT_NEEDS_CANONICAL_NORMALIZATION). Endpoint: `POST /diagnostics/finance-intel/data-foundation-forensics`. 78 tests. Stage 9A — Coverage & Trust Matrix v1. `coverage_trust_matrix_v1.py`: maps Stage 5J LaneCoverage → STRONG/PARTIAL/WEAK/MISSING/NOT_APPLICABLE. Endpoint: `POST /diagnostics/finance-intel/coverage-trust-matrix`. 80 tests.
