...
Last updated: 2026-05-25 (Stage 9F.2a — filing index document selector fix, PR #419 — open). **What changed (this PR):** Root-cause fix for SPY/QQQ/XLE `filing_not_parseable / xml_parse_error`. SEC EDGAR submissions API returns `primaryDocument = "xslFormNPORT-P_X01/primary_doc.xml"` for these ETFs — an XSL transform viewer path that serves HTML, not raw XML. Fix: `_is_xsl_viewer_path()` detects this shape; `_select_best_nport_doc_from_index()` fetches the per-filing index JSON at `https://www.sec.gov/Archives/edgar/data/{cik}/{acc}/{acc}-index.json` and selects the best parseable document (priority: NPORT-typed XML → any XML → SGML .txt → first non-XSL). New diagnostic fields on `NportProviderResult`: `primary_doc_from_submissions`, `selected_doc_source`, `candidate_doc_count`. Diagnostic endpoint now surfaces these 3 fields. 8 new tests (77-83 in test_stage9f2a_etf_nport_holdings.py). Request budget: already 4 (company_tickers + submissions + index + XML); the index fetch is absorbed within the existing cap. **Post-deploy validation steps:** (1) Set `INTEL_V3_NPORT_DIAGNOSTIC_ENDPOINT_ENABLED=true` + `SEC_EDGAR_USER_AGENT=<agent>` in Railway. (2) POST `https://<railway-url>/api/v1/diagnostics/finance-intel/etf-nport-live-check` with header `X-Finance-Runtime-Cert-Secret: <secret>` and body `{"tickers":["SPY","QQQ","XLE","GLD"]}`. (3) Expected: SPY/QQQ/XLE show `fetch_status=success`, `selected_doc_source=index_json`, `holdings_count>0`. (4) GLD should still show `commodity_trust_or_no_nport_data`. **Remaining blockers after this PR:** Vanguard ETFs (VOO/VGT/VYM/VHT/VIS/VTI/VXUS) and SCHD need parent-registrant CIK verification — next slice is ETF parent-registrant CIK resolver if SPY/QQQ/XLE parse successfully. Production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF. **Previous (PR #418):** HTTP diagnostic endpoint added. **Previous (PR #417):** SGML extraction + parse diagnostics.
