...
**Stage 10A product spine reality audit (current PR):** Docs/audit only — no code, UI, SQL, provider migration, synthesis, or decision-policy changes. New `docs/ai/PRODUCT_SPINE_REALITY_AUDIT.md` maps the revised spine onto the actual v2 code.
- **Usable today:** F1 Books of Record (positions/transactions/deposits/DRIP/Plaid/CSV — AVCO cost basis, Plaid authoritative w/ 24h cache, CSV dedup+reconcile), F2 Data Backbone (concurrent price engine yfinance+Finnhub+Alpaca+Polygon+CoinGecko, circuit breakers), Stage 1 Honest Intel (deterministic `decision_policy_v1`, suppression/freshness/credibility/governance — most mature), Stage 4 labeled advisory analyst (single-LLM orchestrator, conviction capped, action deterministic), Stage 2 Deploy exact-dollar math (validated $900/$1,500).
