...
Last updated: 2026-05-22 (Stage 8E — SEC catalyst plain-English explanation layer on top of Stage 8D. New `sec_catalyst_explanation_adapter_v1.py` (pure) converts existing artifact payload fields (catalyst_count, usable_count) into safe optional display strings (event_summary, freshness_label, material_filing_label, limitation_note, decision_authority_note). `intel_v3_service.py` adds `_get_sec_catalyst_artifact_payloads()` (fail-soft SELECT on existing research_artifacts) and merges explanation fields into sec_catalyst_display when sec_catalyst_found=True in both run paths. `SecCatalystEvidenceDisplay` in api.ts extended with optional explanation fields. `buildCatalystEvidenceDisplay()` uses them when available, falls back to generic Stage 8D copy. Clarifies editorial_suppressed body when both flags true. 25 backend + 15 new frontend tests. No SQL, no providers, no LLM, no policy change.)
