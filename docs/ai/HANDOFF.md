...
Last updated: 2026-07-09 (Stage 12E — Paycheck Plan Preview UI; frontend-only; no backend, SQL, LLM, or provider changes).
...
- **New env var required (deploy-time, not code):** `FINANCE_RUNTIME_CERT_SECRET` must be set in the frontend's server environment (same value as the backend's cert secret) for the proxy route to reach the cert-gated backend endpoint. Not committed anywhere; no `.env.example` edit made (file access restricted this session).
...
- **UI validation:** Rendered via a temporary local mock harness (not committed) with the validated Stage 12D response — confirmed VTI first at $2,065.00, SPY at $670.00, allocated $2,735.00, unallocated $2.50, "Plan ready" badge, and the deterministic-guidance caveat. Screenshot captured for PR proof.
- **Files changed:** `v2/frontend/src/lib/paycheck-plan-helpers.ts` (new), `v2/frontend/src/components/cards/PaycheckPlanPreviewCard.tsx` (new), `v2/frontend/src/components/cards/PaycheckPlanPreviewContract.test.ts` (new), `v2/frontend/src/app/api/advisor/paycheck-plan/preview/route.ts` (new), `v2/frontend/src/app/dashboard/paycheck-plan/page.tsx` (new), `v2/frontend/src/components/navigation/BottomNav.tsx` (nav entry added), `docs/ai/HANDOFF.md`, `docs/ai/USAGE_LEDGER.md`.
