...
Last updated: 2026-05-24 (Stage 9E.1 — Equity Numeric Valuation Inputs v1, semantic fix patch, PR #413 open). **What changed:** New pure module `equity_numeric_valuation_inputs_v1.py` builds per-ticker numeric valuation input readiness from `CanonicalEquityDatasetRow` + `TickerPriceSignal`. `TickerPriceSignal` per-ticker price confirmation derived from `portfolio_snapshots.positions_data` JSONB. Freshness: <24h=FRESH, 24-72h=AGING, >72h=STALE. **Semantic fix (current patch):** `numeric_inputs_ready=True` now requires `numeric_price_confirmed=True` (market_price_usd per-share field present — not just market_value_certified_at alone) AND `numeric_earnings_confirmed=True` (EvidenceSectionRecord.latest_period_identity non-None + source_artifact_id non-None — section status alone is NOT sufficient). New readiness scaffold fields on `EquityNumericValuationInputs`: `valuation_input_scaffold_present`, `ticker_price_metadata_present`, `numeric_price_confirmed`, `numeric_earnings_confirmed`. `numeric_price_confirmed` also added to `PriceInput.to_dict()`. `equity_valuation_evidence_v1.py` `_compute_valuation_ready()` now requires `band != BAND_UNKNOWN` (always False at Stage 9E.1 — no thresholds). `intel_data_foundation_forensics_v1.py`: `_extract_ticker_price_signals()` checks `market_price_usd` presence; `valuation_lane_exists` uses `val_row.valuation_ready` (not `numeric_inputs_ready`) so `VALUATION_LANE_NOT_BUILT` gap always remains at Stage 9E.1. `equity_numeric_valuation_ready_count` remains 0 unless true numeric price + earnings confirmed. 307 tests pass (71 Stage 9E.1 + 60 Stage 9E + 75 Stage 9D + 100 Stage 9B). No SQL, no LLM, no providers, no UI, no decision_policy_v1 changes. Next blocker: ETF canonical dataset + valuation lane (parity gate).
