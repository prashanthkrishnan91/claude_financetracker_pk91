...
- **Ticker classification:** Deterministic. Broad ETFs (VOO/VTI/SPY/QQQ), Dividend ETFs (VYM/SCHD), International (VXUS), Sector ETFs (VGT/VHT/VIS/XLE), Alternatives (GLD), Crypto (BTC/ETH/XRP/SOL/DOGE/ADA/…), Speculative (STUB/KLAR/BLSH/RDDT). Unknown tickers default to individual_stock with warning.
- **Policy:** Conservative profile policy v1. ETF floor 40% → prefer ETFs when floor not met. Individual stock cap 20% per ticker. Speculative cap 5% per ticker. Crypto group cap 5% total. Alternatives cap 5% total. Same-theme cap 40% total. No SELL/TRIM generated.
- **Output sections:** diagnostic_version, input, truth_dependency, current_portfolio, generated_policy, target_vs_current (by_group + by_ticker), next_buy_candidates, cash_plan, verdict.
- **Verdict fields:** `policy_status` (ready/degraded/blocked), `recommendations_trusted` (always False), `numeric_plan_trusted` (True if certified truth + no blockers), `next_required_fix`.
- **Reconciliation gate:** Same tolerances as Stage 11A — pass ≤1%, degraded ≤5%, blocked >5%. Stale/missing prices → degraded (not blocked).
- **Per-ticker price queries:** Limit=10 per ticker, ordered desc — extends Stage 11B truncation-protection pattern.
- **Tests:** 67/67 pass (49 service + 9 router + 9 input-validation). Covers cert-gating, cash_to_deploy>0 required, no provider calls, no writes, no LLM calls, certified truth allows policy run, missing/stale prices degrades, reconciliation mismatch degrades/blocks, ETF floor creates ETF buy preference, crypto cap prevents overweight crypto buy, stock cap prevents overweight stock buy, unknown ticker classified as individual_stock with warning, works without Intel v3 using neutral defaults, Intel v3 overlay affects confidence label, min_trade_amount/max_positions enforced, no eligible buy returns no_buy_reason, deterministic output ordering, all required output sections present.
- **No SQL migration.** No target_allocations writes. No recommendation rows created. No deposit_service changes.
- **Files changed:** `v2/backend/app/services/allocation_policy_v1.py` (new), `v2/backend/app/routers/diagnostics.py` (endpoint added), `v2/backend/tests/test_allocation_policy_v1.py` (new), `v2/backend/tests/test_allocation_policy_v1_router.py` (new), `docs/ai/HANDOFF.md`, `docs/ai/USAGE_LEDGER.md`.
