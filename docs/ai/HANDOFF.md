...
Last updated: 2026-05-25 (Stage 9F.2a — ETF NPORT identity-certification repair, open PR). **What changed:** Identity certification lane added to NPORT provider to prevent false positives where VGT/VHT/VIS all returned `success` from the same parent registrant filing (CIK 0000036405) without verifying which series the filing belongs to. Root cause: parent registrant files NPORT-P per series; without identity check, all three tickers blindly accepted the same filing. **Fix:** (1) `etf_parent_cik_resolver.py` extended with 4 new fields on `ETFParentRegistrantEntry`: `expected_series_names`, `standalone_trust`, `commodity_trust`, `candidate_ciks`. SPY/QQQ=`standalone_trust=True`; XLE, all Vanguard, SCHD have expected series name hints. (2) `nport_provider_v1.py` implements identity certification: `_parse_nport_xml` now extracts `seriesName`/`regName`/`seriesId`/`classId` from NPORT genInfo; `_normalize_name()` strips punctuation without space-substitution (U.S.→us matches US→us); `_identity_name_matches()` bidirectional normalized substring; main fetch function iterates candidate CIK list, retrying on `no_nport_filing`/`sec_error`, stopping on mismatch. Identity statuses: `success_identity_verified` (series name matched), `success_identity_assumed_single_series` (standalone trust), `series_identity_not_proven` (no match — returns NO holdings), `commodity_trust_or_no_nport_data` (GLD). 11 new fields on `NportProviderResult` (identity_status, identity_verified, identity_basis, candidate_ciks_tried, selected_candidate_cik, detected_registrant_name, detected_series_name, detected_class_name, detected_series_id, detected_class_id, identity_mismatch_reason). All failure paths now preserve `resolver_source`/`parent_registrant_name` (VYM sec_error diagnostics fix). (3) `nport_diagnostic_runner.py` surfaces 11 identity fields + `tickers_identity_verified` aggregate. (4) `test_stage9f2a_nport_identity_cert.py` — 32 new fixture-based tests (84-116). `test_stage9f2a_etf_parent_cik_resolver.py` — test_76 updated to use XLE series XML fixture, asserts `identity_verified=True`. **145 total Stage 9F.2a tests pass.** **Status: diagnostic validation only** — production flag `INTEL_V3_ETF_NPORT_EVIDENCE_ENABLED` remains OFF. **Post-deploy validation:** POST diagnostic endpoint — success criterion: SPY/QQQ/XLE return `identity_status=success_identity_assumed_single_series` or `success_identity_verified`; VGT/VHT/VIS show `series_identity_not_proven` if parent filing doesn't contain matching seriesName; GLD shows `commodity_trust_or_no_nport_data`.
