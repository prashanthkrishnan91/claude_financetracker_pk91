...
Last updated: 2026-05-23 (Stage 9 architecture contract — docs-only. Added `docs/ai/intel/STAGE_9_RESEARCH_DATA_FOUNDATION_CONTRACT.md`: defines the canonical cross-asset research data foundation required *before* any LLM synthesis. Covers current-state assessment (Intel v3 strengths + the 6 real evidence lanes: sec_company_facts, fundamentals, technicals, news_sentiment, sec_catalyst_sentiment, macro_context), Canonical Research Dataset v1 (separate stock / ETF / crypto shapes), Coverage & Trust Matrix v1 (STRONG/PARTIAL/WEAK/MISSING/NOT_APPLICABLE, mapped from existing Stage 5J/5K statuses), Synthesis Gate v1 (deterministic gate; weak coverage → suppress, never LLM gap-fill), HOLD reason taxonomy (target-weight / watchlist / evidence-gap / valuation-wait / catalyst-wait / risk-contained / better-opportunity), and a 9A–9F build sequence (coverage matrix → stock/ETF/crypto adapters → HOLD reasons → synthesis-gate dry-run). Synthesizer/LLM/paid providers explicitly deferred. No product code changed, no SQL, no LLM, no providers, no policy change, safe_for_decision stays False. Prior entry: Stage 8F — SEC filing-type specificity layer on top of Stage 8E. Pre-build gate confirmed: form types stored in research_artifact_sources.section_reference (existing data, no schema change). New `sec_filing_type_adapter_v1.py` (pure) maps section_reference values → plain-English filing_type_label (10-K→"Annual report (10-K)", 10-Q→"Quarterly report (10-Q)", 8-K→"Company event filing (8-K)", multiple→"Multiple recent official filings", unknown→"Official company filing", empty→{}). `stage8f_filing_type_contract_v1.py` contract marker + staleness helper. `intel_v3_service.py`: `_get_sec_catalyst_artifact_data()` replaces `_get_sec_catalyst_artifact_payloads()` — second SELECT on research_artifact_sources for section_references; both run paths updated to call `_build_filing_type_display()`; stage8f added to recertification cascade in `enqueue_run_v3()`. `snapshot_builder.py` adds `stage8f_filing_type_contract_version` marker. Frontend: `SecCatalystEvidenceDisplay` gets optional `filing_type_label`; `CatalystEvidenceItem` gets optional `filing_type_label`; `buildCatalystEvidenceDisplay()` passes it through. 46 backend + 14 new frontend tests. No SQL migrations, no schema changes, no providers, no LLM, no policy change.)
