...
Last updated: 2026-05-23 (Stage 9A — Coverage & Trust Matrix v1 backend foundation. New `coverage_trust_matrix_v1.py` (pure mapper, no I/O): maps Stage 5J `LaneCoverage` statuses → STRONG/PARTIAL/WEAK/MISSING/NOT_APPLICABLE per 10 research categories (fundamentals, valuation, technicals, news_sentiment, sec_company_facts, sec_catalysts, etf_fund_composition, crypto_market_context, portfolio_sizing_target_weight, thesis_history). Asset-type rules: SEC/company facts and SEC catalysts NOT_APPLICABLE for ETF and crypto; etf_fund_composition MISSING (no fund-data lane) for ETF, NOT_APPLICABLE for others; fundamentals and valuation NOT_APPLICABLE for crypto; crypto_market_context capped at PARTIAL (price-as-proxy) for crypto, NOT_APPLICABLE for others. Lane→state mapping: READY + primary/official authority → STRONG; READY + free/unofficial / LIMITED → PARTIAL; SUPPRESSED/STALE_OR_UNKNOWN/NOT_EVALUABLE → WEAK; MISSING → MISSING. Per-category output: state, reason, source_reference, allowed_for_synthesis, suppression_reason. Ticker summary: asset_type, synthesis_ready=False, allowed_blocks, suppressed_blocks, strongest_gaps. Config: `intel_v3_coverage_trust_matrix_enabled` flag (default False). Endpoint: `POST /diagnostics/finance-intel/coverage-trust-matrix` (cert-gated + flag-gated; falls back to positions for tickers; read-only; 200-ticker cap; hard-locks safe_for_decision=False and synthesis_ready=False). Tests: 80 new in `test_stage9a_coverage_trust_matrix.py` (mapping, equity/ETF/crypto/unknown rules, synthesis allowance invariants, suppressed news stays WEAK, aggregate summary, safety/leak guard, decision policy regression, multi-asset portfolio, symbol fallback). No SQL, no LLM, no providers, no UI, no policy change, safe_for_decision=False throughout. Prior entry: Stage 9 architecture contract — docs-only PR #407 establishing `docs/ai/intel/STAGE_9_RESEARCH_DATA_FOUNDATION_CONTRACT.md` (cross-asset research data foundation, Canonical Research Dataset v1, Coverage & Trust Matrix v1 design, Synthesis Gate v1, HOLD reason taxonomy, 9A–9F build sequence).
