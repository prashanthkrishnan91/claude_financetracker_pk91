...
> produced this PR cannot stream a 378 KB file rewrite through a single tool call.
...
That contradiction caused bloated prompts and slow tiny PRs (excessive micro-phasing, repeated workflow boilerplate inside prompts, all-agent default reviews).

### What changed (docs-only)
- **CLAUDE.md** — added Prompt Compression and Capability Slices section (task delta only; safety packs and archetypes own repeated rules; <700–1,200 word compression budget; capability slice over micro-patch). Condensed conflicting rules.
- **docs/ai/AI_REPO_OPERATING_SYSTEM.md** — added new consolidation section *inside existing OS v4* (no v4.2/v5 label): default prompt shape, what belongs in prompt vs repo-native, batch-vs-split decision gate, meaningful progress standard, and test tier hierarchy.
...
- **docs/ai/PROMPT_LIBRARY.md** — added compact templates (Capability Slice, Backend Scaffold / Promotion Gate, Sev 1 Plumbing Fix, UI Surface Pass with Budget, PR Merge Gate, Workflow Update). Updated reviewer gate with compression / batch-vs-split / tiered-test questions and named-safety-pack check.
...
- **docs/ai/SAFETY_PACKS_AND_ARCHETYPES.md** — new: 9 shared safety packs (No Visible Behavior Change, Backend-only Scaffold, Runtime/API Contract, No Provider/LLM Expansion, Plain-English UI, Evidence/Claim Safety, SQL/Persistence Manual Action, Performance/Latency, Test Tier), 4 Finance-specific packs (Deterministic Decision Authority, Valuation Safety, Data Truth / Evidence Suppression, Deploy/Watchtower Boundary), and 9 shared build archetypes (capability-slice, disabled-promotion-scaffold, shadow-to-visible-governance, full-plumbing-root-cause-fix, contract-consolidation, runtime-validation, UI-surface-pass, merge-gate, workflow-update).
- **docs/ai/TEST_ROUTING.md** — new (Finance): Tier 0–3 routing mirroring Travel philosophy (changed-file adjacent, Intel v3 contract / policy / adapter bundles, snapshot/runtime boundary bundles, full suite only at release / shared infra / schema / suspicious-failure boundaries).
...
- test tier hierarchy exists,
- Finance now has `TEST_ROUTING.md`,
