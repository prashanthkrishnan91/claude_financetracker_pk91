...
any feature spec, Intel/Deploy/Watchtower contract, etc.>
...
- Runtime log excerpts, SQL results, or certification responses.
...
- Do not repeat all Finance invariants unless the task is risky.
- Do not ask for broad discovery when a targeted skill is enough.
- Do not combine unrelated policy, UI, worker, SQL, env, and docs work in one prompt.
...
- Do not ask reviewers to report only blockers at the start; use coverage-first audit.
- Do not draft prompts that imply LLM/research final visible action authority.
