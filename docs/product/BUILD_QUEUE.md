...
- **Stage 2 exit validation (re-run)** (Stage 2, active). Stages 2.5A–2.6D complete. Re-validate the full Step 1/2/3 flow in production with a real amount (e.g. $900): Step 2 must show **3–5 amount-aware BUY recommendations** deploying a meaningful share of the entered cash (not the previous 2-BUY $59-of-$900 result), and Step 3 must log the correct amount-aware Deploy v3 decision **and persist after refresh**. Expected production log: `deploy_v3.plan ... amount_aware=True cash_to_deploy=900 residual_cash=... residual_reason=...`. Readiness gates must be green. Stage 2 stays open until production shows a useful 3–5 amount-aware BUY recommendation set and Step 3 logging persists after refresh. Do not mark Stage 2 complete or move to Stage 3 until then.
