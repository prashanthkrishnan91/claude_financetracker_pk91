...
- **Stage 2 exit validation (re-run)** (Stage 2, active). Stages 2.5A–2.7 complete. Re-validate the full Step 1/2/3 flow in production with a real amount (e.g. $900). Stage 2 exit remains pending until **all four** are observed: (1) amount-aware Deploy v3 shows 3–5 BUY recommendations; (2) Step 3 lets the user edit actual amounts (NFLX $245 → $250) and add a manual NVDA BUY $100 row, with `recommended_amount` preserved and `is_manual` correctly set on manual rows; (3) decision log history renders below Step 3 and persists after refresh, with re-saving the same active plan updating the latest log rather than creating duplicates; (4) recommendation confidence/ranking explanation is reviewed in a later evidence-quality slice (deferred — not in 2.7). Readiness gates must be green. Do not mark Stage 2 complete or move to Stage 3 until all four are validated in production.
