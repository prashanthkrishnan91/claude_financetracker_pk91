...
USAGE_CLAIM_RE = re.compile(
    r"usage\s+(tracked|ledger\s+updated|ledger.*yes)|"
    r"see\s+usage\s+ledger|"
    r"ledger\s+updated.*yes",
    re.IGNORECASE,
)
...
class Checker:
    ...
    def check_ledger(self) -> None:
        ...
        if lchanged and self.ledger_text:
            recent = [l for l in self.ledger_text.splitlines()[-20:] if l.startswith("|")]
            if any("unknown" in l.lower() for l in recent):
                self._warn(
    ...
    def check_deps(self) -> None:
        if self.files is None or not self.body:
            return
        env_files = [f for f in self.files if ENV_FILE_RE.search(f)]
        if env_files:
            self._fail(f"Committed .env/secrets file detected: {', '.join(env_files)}. Remove immediately.")
...
def run_self_tests() -> int:
    ...
    eq("claims_usage_yes", claims_usage("Usage tracked: yes"), True)
    eq("claims_usage_ledger", claims_usage("Usage ledger updated: Yes"), True)
    eq("claims_usage_no", claims_usage("No usage info"), False)
...
def main() -> int:
    ...
    code = checker.run()
    if args.fmt == "json":
        print(json.dumps(checker.result_json(), indent=2))
    return code
