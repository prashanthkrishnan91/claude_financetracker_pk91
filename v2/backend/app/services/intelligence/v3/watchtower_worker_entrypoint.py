...
def _build_default_price_refresh_callable(client: Any) -> Optional[Any]:
    """Delegate to shared watchtower_callables_v1 — kept for backward compat."""
    from .watchtower_callables_v1 import build_default_price_refresh_callable
    return build_default_price_refresh_callable(client)


def _build_default_analyst_enqueue_callable(client: Any) -> Any:
    """Delegate to shared watchtower_callables_v1 — kept for backward compat."""
    from .watchtower_callables_v1 import build_default_analyst_enqueue_callable
...
def _build_default_alert_candidate_hook_callable(client: Any) -> Any:
    """Delegate to shared watchtower_callables_v1."""
    from .watchtower_callables_v1 import build_default_alert_candidate_hook_callable
    return build_default_alert_candidate_hook_callable(client)
...
async def _run_cycle_for_all_users(client: Any) -> dict[str, Any]:
    ...
    alert_candidate_hook = _build_default_alert_candidate_hook_callable(client)
    ...
    for user_id in user_ids:
        try:
            worker = WatchtowerBackgroundRefreshWorker(
                ...
                alert_candidate_hook_callable=alert_candidate_hook,
            )
