...
# ── A1c (superseded) ─────────────────────────────────────────────────────────
# The router-level "recognize existing durable work" tests that lived here
# encoded the pre-session architecture: an unscoped drain plus completion
# inferred from the globally-latest snapshot. That path was replaced by
# durable SQL-backed run sessions (intel_run_session_flow_v1) — see
# test_run_intel_session_flow.py and test_run_intel_session_router.py for the
# replacement coverage (session-scoped drains, session-linked publication,
# completion truth). The job-store / worker / drain scoping tests above are
# unchanged production behavior and remain authoritative.
