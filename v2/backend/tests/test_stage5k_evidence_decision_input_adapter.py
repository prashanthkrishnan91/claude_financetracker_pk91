...
class TestEndpointCertGate:
    """Endpoint must be blocked when flag is off."""

    def test_flag_off_raises_403(self) -> None:
        import asyncio
        from types import SimpleNamespace
        from unittest.mock import patch
        from fastapi import HTTPException
        from app.routers.diagnostics import get_research_evidence_decision_readiness, ResearchEvidenceDecisionReadinessRequest
        ...
        settings = SimpleNamespace(
            ...
            intel_v3_evidence_decision_readiness_diagnostics_enabled=False,
        )
        fake_user = SimpleNamespace(id=_uid)
        req = ResearchEvidenceDecisionReadinessRequest(tickers=["AAPL"])

        with patch("app.routers.diagnostics.get_settings", return_value=settings):
            with pytest.raises(HTTPException) as exc_info:
                asyncio.get_event_loop().run_until_complete(
                    get_research_evidence_decision_readiness(payload=req, user=fake_user)
                )
            assert exc_info.value.status_code == 403

    def test_cert_flag_disabled_raises_404(self) -> None:
        from fastapi import HTTPException
        ...
        settings = SimpleNamespace(
            ...
            finance_runtime_cert_secret="s",
        )
        with patch("app.routers.diagnostics.get_settings", return_value=settings):
            with pytest.raises(HTTPException) as exc_info:
                _ensure_cert_enabled("s")
            assert exc_info.value.status_code == 404


class TestEndpointReadOnlyAndSafe:
    """End-to-end endpoint call must be read-only and free of leaks."""

    def _run_endpoint(self, tickers: list[str], artifact_rows: list[dict]) -> Any:
        import asyncio
        from types import SimpleNamespace
        from unittest.mock import patch
        from app.routers.diagnostics import (
            get_research_evidence_decision_readiness,
            ResearchEvidenceDecisionReadinessRequest,
        )
        ...
        req = ResearchEvidenceDecisionReadinessRequest(tickers=tickers)

        with patch("app.routers.diagnostics.get_settings", return_value=settings):
            with patch("app.routers.diagnostics.get_supabase_client", return_value=_FakeClient(db)):
                result = asyncio.get_event_loop().run_until_complete(
                    get_research_evidence_decision_readiness(payload=req, user=fake_user)
                )
        return result, db

    def test_safe_for_decision_false_in_response(self) -> None:
        result, _ = self._run_endpoint(["AAPL"], [_sec_row("AAPL")])
        assert result["safe_for_decision"] is False

    def test_shadow_only_true_in_response(self) -> None:
        result, _ = self._run_endpoint(["AAPL"], [])
        assert result["shadow_only"] is True

    def test_no_raw_payload_in_response(self) -> None:
        result, _ = self._run_endpoint(
            ["AAPL"], [_sec_row("AAPL", secret_field="ENDPOINT_SECRET")]
        )
        blob = json.dumps(result)
        assert "ENDPOINT_SECRET" not in blob
        assert "api_key" not in blob
        assert '"payload"' not in blob

    def test_no_db_writes_from_endpoint(self) -> None:
        result, db = self._run_endpoint(["AAPL"], [_sec_row("AAPL"), _macro_row()])
        assert db.write_attempts == 0

    def test_etf_sec_not_applicable_in_response(self) -> None:
        result, _ = self._run_endpoint(["SPY"], [])
        tr = result["ticker_readiness"]["SPY"]
        assert tr["sec_lane_applicable"] is False
        fund_axis = tr["axes"][AXIS_COMPANY_FUNDAMENTALS]
        assert LANE_SEC_COMPANY_FACTS in fund_axis["not_applicable_lanes"]
        assert LANE_SEC_COMPANY_FACTS not in fund_axis["missing_lanes"]
