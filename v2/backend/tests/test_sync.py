...
# ── DRIP Dividend Date Helper ─────────────────────────────────────────────────


class TestGetDividendDates:
    """Tests for _get_dividend_dates — yfinance calendar wrapper.

    yfinance is NOT installed in the test environment (heavy library).
    We inject a mock yfinance module via sys.modules so the lazy
    `import yfinance as yf` inside _get_dividend_dates gets our mock.
    """

    def _clear_cache(self):
        from app.services.drip_service import _dividend_date_cache
        _dividend_date_cache.clear()

    def setup_method(self):
        self._clear_cache()

    def _mock_yf_module(self, calendar_data):
        """Return a context manager that injects a mock yfinance module."""
        mock_yf = MagicMock()
    ...
    def test_dict_calendar_with_dates(self):
        from datetime import date
        from app.services.drip_service import _get_dividend_dates

        cal = {
            "Ex-Dividend Date": date(2026, 3, 19),
            "Dividend Date": date(2026, 3, 27),
        }
        ctx, _ = self._mock_yf_module(cal)
        with ctx:
            ex, pay = _get_dividend_dates("VYM")

        assert ex == "2026-03-19"
        assert pay == "2026-03-27"

    def test_dict_calendar_none_values(self):
        from app.services.drip_service import _get_dividend_dates

        cal = {"Ex-Dividend Date": None, "Dividend Date": None}
        ctx, _ = self._mock_yf_module(cal)
        with ctx:
            ex, pay = _get_dividend_dates("SCHD")
    ...
    def test_empty_calendar(self):
        ...
        ctx, _ = self._mock_yf_module(None)
        with ctx:
            ex, pay = _get_dividend_dates("SPY")
    ...
    def test_result_is_cached(self):
        from datetime import date
        from app.services.drip_service import _get_dividend_dates, _dividend_date_cache

        cal = {
            "Ex-Dividend Date": date(2026, 3, 19),
            "Dividend Date": date(2026, 3, 27),
        }
        ctx, mock_yf = self._mock_yf_module(cal)
        with ctx:
            _get_dividend_dates("VYM")
            _get_dividend_dates("VYM")  # second call — should hit cache

        # yfinance.Ticker called only once — second call served from cache
        assert mock_yf.Ticker.call_count == 1
        assert "VYM" in _dividend_date_cache

    def test_nan_string_treated_as_none(self):
        from app.services.drip_service import _get_dividend_dates

        cal = {"Ex-Dividend Date": "nan", "Dividend Date": "NaT"}
        ctx, _ = self._mock_yf_module(cal)
        with ctx:
            ex, pay = _get_dividend_dates("BND")

        assert ex is None
        assert pay is None

    def test_iso_string_date(self):
        """Some yfinance versions return date strings directly."""
        from app.services.drip_service import _get_dividend_dates

        cal = {"Ex-Dividend Date": "2026-03-19", "Dividend Date": "2026-03-27 00:00:00"}
        ctx, _ = self._mock_yf_module(cal)
        with ctx:
            ex, pay = _get_dividend_dates("SCHD")
    ...
    def test_different_tickers_cached_separately(self):
        ...
        cal_vym = {"Ex-Dividend Date": date(2026, 3, 19), "Dividend Date": date(2026, 3, 27)}
        cal_schd = {"Ex-Dividend Date": date(2026, 3, 20), "Dividend Date": date(2026, 3, 28)}

        ticker_map = {"VYM": cal_vym, "SCHD": cal_schd}

        mock_yf = MagicMock()

        def make_ticker(symbol):
            mock_t = MagicMock()
            mock_t.calendar = ticker_map.get(symbol)
            return mock_t

        mock_yf.Ticker.side_effect = make_ticker

        with patch.dict("sys.modules", {"yfinance": mock_yf}):
            ex_vym, _ = _get_dividend_dates("VYM")
            ex_schd, _ = _get_dividend_dates("SCHD")

        assert ex_vym == "2026-03-19"
        assert ex_schd == "2026-03-20"
        assert "VYM" in _dividend_date_cache
        assert "SCHD" in _dividend_date_cache
