...
function normalizeAnalystAction(action?: string | null): string | null {
  if (!action) return null;
  const raw = action.toUpperCase();
  if (raw === "REDUCE") return "TRIM";
  if (raw === "INSUFFICIENT_DATA") return "HOLD";
  return raw;
}
...
export function AgentInsightCard({
  ...
  const normalizedAction = normalizeAnalystAction(card.analyst_action) || card.action;
  const subtitle = `${card.category} · ${card.sector || "Unknown"}`;
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span className={cn("px-2 py-0.5 rounded text-xs font-bold uppercase border", styles.bg, styles.text, styles.border)}>
            {normalizedAction}
          </span>
          <div className="flex items-center gap-2 text-[10px] uppercase">
            <span className="text-text-muted">
              <span className={cn("inline-block w-1.5 h-1.5 rounded-full mr-1 align-middle", card.analysis_source === "live_llm" ? "bg-green-400" : "bg-text-muted")} />
              {card.analysis_source === "live_llm" ? "Live" : "Cached"}
            </span>
            {card.data_quality_label && (
              <span className={cn("font-semibold", QUALITY_STYLES[card.data_quality_label] || "text-text-muted")}>
                {card.data_quality_label}
              </span>
            )}
          </div>
      ...
      {(card.analyst_drivers?.length || card.analyst_risks?.length || card.what_changed) ? (
