...
- Phase 2.1 promotion changes vs. draft: (1) `worker_audit_events` user_id consistency trigger added; (2) JSONPath column alias renamed `t(value)` → `kv(obj)`; (3) PG 14+ compatibility documented; (4) DB-level hard lock `research_artifacts_safe_for_decision_phase2_chk CHECK (safe_for_decision = FALSE)` — workers cannot set decision-safe; Phase 4/5 truth-adapter migration must DROP this constraint; (5) `research_artifact_facts.source_id` provenance verified in trigger facts branch — source must belong to same `artifact_id` and `user_id`, blocking cross-artifact citation.
